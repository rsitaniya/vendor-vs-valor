...
- **degraded** — the output was invalid/ungrounded (unknown ids, uncited, out-of-pool, missing gate evidence, or a generation error). The challenger is degradable, so this **never fails the stage**, but the reason is recorded in `strategy.json` and rendered in `strategy.md` (visible, not silent — Contract rule 12); the runner-up falls back to synthesis' own second-best.
