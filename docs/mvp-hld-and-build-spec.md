...
1. **Undated-source flag, not a hard reject**: `assert_claim` accepts a cost-tagged claim whose sources lack `source_date` (many vendor/OSS pricing pages are JS-rendered and have no extractable publish date); `filter` marks it `undated_cost` instead, kept but visibly flagged. (Revised from an earlier hard-reject rule, PR-001 F3, after real runs showed the reject was silently starving cost dimensions of evidence rather than surfacing a usable flag.)
...
**(b) `challenger` — LLM, adversarial path-advocate (degradable).** Handed the **need profile**, the recommendation, **and the evidence pools** (not just the conclusion, otherwise it hallucinates a counter-case), it makes the strongest **cited** case for a *different* path, under the same `grounded_claim` discipline (every counter-claim is a verified `Claim`). The result resolves to one of **three recorded states**, never a silent swallow:

- **mounted**: a distinct, grounded counter-recommendation. It clears the **same gates synthesis must**: cited ids exist and are non-empty, every cited claim comes from the counter path's own evidence pool(s) (§3.3), and a `buy_then_extend` counter carries a BUY-side `m10` API-surface claim. A mounted counter is presented as a **first-class counter-recommendation** (its `wins_when` conditions are reasoned from the profile). If it independently lands on synthesis' own second-best, the convergence is noted rather than double-rendered.
- **concurred**: the challenger returned the **recommended** path: the adversarial pass could not beat it. This is signal (earned confidence), not failure; it is surfaced as such, and the runner-up falls back to synthesis' own second-best.
- **degraded**: the output was invalid/ungrounded (unknown ids, uncited, out-of-pool, missing gate evidence, or a generation error). The challenger is degradable, so this **never fails the stage**, but the reason is recorded in `strategy.json` and rendered in `strategy.md` (visible, not silent, per Contract rule 12); the runner-up falls back to synthesis' own second-best.

**Degradation:** if the sprint runs short, the challenger is disabled and the runner-up comes from a single synthesis pass; it is the first thing to drop and the structure is unchanged.
