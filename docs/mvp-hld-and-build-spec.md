...
4. `synthesize` emits `strategy.md` with: a **recommendation** + thesis; per-path **dossiers** (pros/cons/risks/reversibility) for the four paths, each claim cited; a **runner-up** with "wins when" conditions; **decisive factors**; and **open questions**.
...
**(a) `synthesize` — LLM, four-path lenses.** Reads `profile.json` + both filtered pools + `paths.json`. For each of the four paths, reasons over the **mapped** evidence pool(s) (§3.3) and writes a **dossier**: pros, cons, key risks, **reversibility note** (Engine Rule #6), each factual claim citing a verified `Claim`. Produces the **recommendation** + 2–3 sentence thesis and the **decisive factors** (the 3–5 dimensions that drove it — this is the weight-replacement, §Design 3.4). Surfaces **open questions** for `null`/thin evidence (stated as gaps, never invented).
...
**(c) assemble — CODE.** Validates that the synthesis output has one dossier per canonical path, that runner-up differs from the recommendation, that every cited claim id resolves to the verified evidence pool, and that a buy-then-extend recommendation cites BUY-side `m10` API/extensibility evidence. Broken citations or missing gate evidence fail the stage; they are never silently dropped. Then it combines recommendation + dossiers + runner-up + decisive factors + open questions into `strategy.md` (the deliverable) and a `strategy.json` sidecar (section manifest + claim-ids) for the renderer. No new model call.
