...
> An advisory intelligence layer that turns capability sourcing from a hallway conversation into a grounded, citation-backed recommendation. It first understands intent, then researches internal and vendor-led paths autonomously, then reasons over four strategic options, and argues against its own answer before it ships.
...
- **Is not a vendor-ranking SEO tool.** It's grounded research, not a Gartner clone.
...
- **Generic deep-research capability** → **adopt / buy.** Open-source deep-research agents and orchestration frameworks (LangGraph and its ecosystem) are mature, MCP-native, benchmarked, commoditized. Rebuilding this is wasted capital. *Verdict: adopt, using LangGraph for orchestration and a hosted model API for inference.*
...
- **Net path: buy-then-extend.** Adopt the orchestration and model primitives; build the differentiating layer on top. It's exactly the path the engine is designed to surface, and the path modern stacks converge on.
...
Two independent choices: **orchestration substrate** and **model provider**. Both are made, and both sit behind thin interfaces so they stay reversible. The engine practices the reversibility it preaches.
...
**The decision criterion, stated honestly:** minimize reversibility cost and let the framework own only the commodity. Both choices are reversible by construction (interfaces), so this is "decided, but not married." That's the correct senior posture.
