...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Vendor vs Valor LangGraph pipeline from input files.",
    )
    parser.add_argument(
        "--input",
        required=True,
        help="Path to the capability input file, resolved from the repo root when relative.",
    )
    parser.add_argument(
        "--context",
        default=None,
        help="Optional context file, resolved from the repo root when relative.",
    )
    parser.add_argument(
        "--runs-root",
        default="runs",
...
def run_pipeline(args: argparse.Namespace) -> int:
    ...
    context_path = resolve_root_path(args.context) if args.context else None
    ...
    context_text = read_input_file(context_path) if context_path else ""
