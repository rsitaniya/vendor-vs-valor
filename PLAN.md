...
- `/agents/synthesis.md`: four-path lenses over the two pools per `paths.json`; per-path dossier with citation-bound pros/cons/risks/**reversibility** bullets, recommendation + thesis, **decisive factors**, **open questions**. Each factual bullet cites a verified `Claim`.
