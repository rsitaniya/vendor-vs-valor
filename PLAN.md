...
- Deterministic `assemble` (code) → `strategy.md` (truth) + `strategy.json` (section manifest + claim-ids). Gate-3 `interrupt()`; editing soft_steer re-runs synthesis only (hash guard), research skipped.
