...
1. `mvp-hld-and-build-spec.md` — PRIMARY. Contracts (§3 `grounded_claim`, `schema_stage`; §4 run dir + hash guard; §5 stages), build order (§8).
2. `design-and-decision-model.md` — the why, the decision model, the engine rules (§6).
3. `target-architecture.md` — CONTEXT ONLY. Do not build Target features. Use it only to keep MVP seams clean.
...
5. Stage 2 research (one parameterized track, run BUILD then BUY) wired through `grounded_claim`; verify+filter. **[CHECKPOINT 4 — this is the schedule risk; show me real cited output]**
...
- **Use subagents** to investigate in parallel where it helps: e.g. one to probe the Gemini structured-output API and one to probe LangGraph checkpointer/interrupt patterns, while you draft the skill interfaces. Don't fan out on the actual build — slices are sequential.
...
## What NOT to do
- Don't build any Target-Architecture feature (portfolio memory service, parallel fan-out, structural-at-emission grounding, confidence scoring, provider routing, served dashboard). Stubs/interfaces only where the MVP spec says so.
